...
class TestTheatreModels(TestCase):
    ...
    def test_theatre_hall_str(self):
        self.assertEqual(
            ...
            f"Hall {self.theatre_hall.name} " f"has {self.theatre_hall.capacity} seats",
        )
...
class TicketModelTest(TestCase):
    ...
    def test_ticket_validation_failure_row(self):
        ticket = Ticket(
            row=11, seat=1, performance=self.performance, reservation=self.reservation
        )
    ...
    def test_ticket_validation_failure_seat(self):
        ticket = Ticket(
            row=1, seat=11, performance=self.performance, reservation=self.reservation
        )
        with self.assertRaises(ValidationError):
            ticket.clean()
